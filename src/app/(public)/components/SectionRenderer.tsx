...
export default function SectionRenderer({ section, content, template }: SectionRendererProps) {
    ...
    // Check if template has a repeater field
    const repeaterField = template.fields?.find((f: any) => f.type === 'repeater')

    if (repeaterField) {
        // REPEATER RENDERING

        // Special case: Users table (Qui sommes nous)
        if (section.table_name === 'users') {
            return (
                <section id={section.slug} className="scroll-mt-24 py-16 bg-gray-50">
                    <div className="container mx-auto px-4">
                        <h2 className="text-3xl font-bold mb-12 text-gray-900">{section.title}</h2>

                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {content.map((user) => (
                                <div key={user.id} className="bg-white p-8 rounded-lg shadow-sm hover:shadow-md transition-shadow text-center">
                                    {user.avatar_url && (
                                    ...
                                    <p className="text-gray-600 leading-relaxed text-sm">
                                        {lang === 'fr' ? user.bio_fr : user.bio_en}
                                    </p>
                                </div>
                            ))}
                        </div>
                ...
                <section id={section.slug} className="scroll-mt-24 py-16 bg-gray-50">
                    <div className="container mx-auto px-4">
                        ...
                        <div className="space-y-8">
                            {content.map((step) => {
                                const stepContent = step.content || step
                                return (
                                    <div key={step.id} className="flex gap-6 items-start">
                                        ...
                                        <div className="flex-1">
                                            <h3 className="text-2xl font-semibold mb-2 text-gray-900">
                                                Étape {step.step_number} : {getLocalizedValue(stepContent, 'step_title')}
                                            </h3>
                    ...
                    <div className="container mx-auto px-4">
                        ...
                        <div className="space-y-4 max-w-3xl mx-auto">
                            {content.map((item) => {
                                ...
                                return (
                                    <details key={item.id} className="bg-white p-6 rounded-lg shadow-sm group">
        ...
        // Special case: Logos / Trust badges
        if (section.template_slug === 'section_logos_confiance') {
            return (
                <section id={section.slug} className="scroll-mt-24 py-16 bg-gray-50">
                    <div className="container mx-auto px-4">
                        <h2 className="text-3xl font-bold mb-8 text-gray-900 text-center">{section.title}</h2>

                        <div className="flex flex-wrap items-center justify-center gap-8">
                            {content.map((item) => {
                                const itemContent = item.content || item
                                return (
                                    <div key={item.id} className="grayscale hover:grayscale-0 transition-all opacity-60 hover:opacity-100">
                                        {itemContent.logo && (
                                            <Image
                                                src={itemContent.logo}
                                                alt={itemContent.name || 'Logo partenaire'}
                                                width={150}
                                                height={80}
        ...
        // Standard repeater sections (Offres, Tarifs, etc.)
        return (
            <section id={section.slug} className="scroll-mt-24 py-16 bg-gray-50">
                <div className="container mx-auto px-4">
                    <h2 className="text-3xl font-bold mb-8 text-gray-900">{section.title}</h2>
                        ...
                        {content.map((item) => {
                            ...
                            return (
                                <div key={item.id} className="bg-white p-6 rounded-lg shadow-sm">
                                    ...
                                    {/* Subtitle (if exists, for pricing) */}
                                    ...
                                    {/* Price HT (for pricing section) */}
                                    {item.price_ht !== undefined && item.price_ht !== null && (
    ...
    // SIMPLE RENDERING (single content item like "Notre travail")
        ...
        <section id={section.slug} className="scroll-mt-24 py-16 bg-gray-50">
            <div className="container mx-auto px-4">
                ...
                <div className="bg-white p-6 rounded-lg shadow-sm">
                    {/* Body only - no title to avoid duplication */}
