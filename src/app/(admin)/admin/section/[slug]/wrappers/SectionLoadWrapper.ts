...
export async function loadSection(slug: string): Promise<LoadedSectionData> {
    ...
    } else if (["content_offers", "content_pricing", "content_custom_sections"].includes(section.table_name)) {
        ...
        if (repeaterField) {
            formData = {
                [repeaterField.name]: rows.map((row) => ({
                    ...
                    ...(row.price_ht !== null && row.price_ht !== undefined && { price_ht: row.price_ht }),
                    ...(row.price_ttc !== null && row.price_ttc !== undefined && { price_ttc: row.price_ttc }),
                    ...(row.tva_rate !== null && row.tva_rate !== undefined && { tva_rate: row.tva_rate }),
                    ...(row.offer_id !== null && row.offer_id !== undefined && { offer_id: row.offer_id }),
