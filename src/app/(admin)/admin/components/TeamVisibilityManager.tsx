...
export default function TeamVisibilityManager() {
    ...
    const toggleVisibility = async (userId: string, currentVisibility: boolean) => {
        const supabase = supabaseBrowser()
        ...
        if (error) {
            ...
            setUsers(users.map(u => (u.id === userId ? { ...u, is_public: !currentVisibility } : u)));
    ...
    return (
        <div className="space-y-4">
            <div className="bg-blue-50 border border-blue-200 text-blue-800 p-4 rounded-lg mb-6">
                <p className="text-sm">
                    💡 <strong>Gestion de l'équipe :</strong> Activez/désactivez la visibilité des profils sur la page publique "Qui sommes‑nous".
                    Pour modifier votre propre profil, allez dans <a href="/admin/profil" className="underline font-semibold">Mon Profil</a>.
                </p>
            </div>

            {users.length === 0 ? (
                <div className="text-center py-8 text-gray-500">Aucun utilisateur approuvé</div>
            ) : (
                    ...
                    {users.map(user => (
                        ...
                        >
                            ...
                            <div className="flex-shrink-0">
                                {user.avatar_url ? (
                                    <Image src={user.avatar_url} alt={user.pseudo || user.firstname || 'Avatar'} width={60} height={60} className="w-16 h-16 rounded-full object-cover" />
                                ) : (
                            ...
                            <div className="flex-1">
                                ...
                                {user.bio_fr && (
                                    <p className="text-sm text-gray-500 mt-1 line-clamp-2">{user.bio_fr}</p>
                                )}
                            </div>
