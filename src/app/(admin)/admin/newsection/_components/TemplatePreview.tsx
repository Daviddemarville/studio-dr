...
function RepeaterPreview({ field }: { field: any }) {
    ...
    return (
        <div
            className={`grid gap-4 ${count === 1 ? "grid-cols-1" : count === 2 ? "grid-cols-2" : "grid-cols-3"
                }`}
        >
            {[...Array(count)].map((_, i) => (
                <div key={i} className="bg-gray-800 p-4 rounded-lg space-y-3 border border-gray-700">
