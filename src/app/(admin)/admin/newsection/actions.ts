...
export async function createSection(title: string, templateSlug: string, position: number = 0, icon: string = "FileText") {
    ...
    const tableName = TEMPLATE_TABLE_MAP[templateSlug] || "content_custom_sections";
...
export async function deleteSection(id: number) {
    ...
    if (section.is_system) {
        return { success: false, error: "Impossible de supprimer une section système. Vous pouvez la désactiver (rendre invisible) si nécessaire." };
    }
