...
const pseudoNonEmptySchema = z
    ...
    .refine((val) => !/[À-ÖØ-öø-ÿ]/.test(val), "Les accents ne sont pas autorisés.")
...
export const passwordSchema = z
    ...
    .refine((v) => /[!@#$%^&*()_\-+.?]/.test(v), "Doit contenir un caractère spécial.")
    .refine((v) => !/[À-ÖØ-öø-ÿ]/.test(v), "Pas d'accents.")
    .refine((v) => !/[^\u0000-\u00ff]/.test(v), "Pas d'emojis.")
    ...
    .refine(
        (val) => {
            try {
                ...
                if (lower.includes("<script") || lower.includes("%3cscript")) return false;
